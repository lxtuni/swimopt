...
## 推荐用法：双击 `8_控制面板.bat`（图形界面，全都装在里面）

一个窗口搞定：选/导入模型 → 勾选要搜的参数 → 设置预算 → 开始寻优（可边跑边看）→ 看收敛曲线和最优参数表。

**面板四块**：
1. **模型**：下拉选 `robots/` 里的模型；`导入 URDF…` 一键转换并自动启用
2. **搜索空间**：勾选 `频率 / 幅度 / 相位 / 偏置` —— **取消勾选=冻结该参数**，右侧实时显示"优化 X/Y 维"
   - 想固定步态只搜频率和幅度？→ **取消"相位"和"偏置"**，在下拉里选 `diag对角 / fb前后 / lr左右 / wave行波 / inphase同相`
3. **运行设置**：评估次数、每次仿真秒数、种群大小、随机种子、可视化模式（每组看/每5组/只看纪录/不看）
4. **结果**：左侧表格列出**每个电机的运动规律**（偏置/1次幅度相位/2次幅度相位），右侧**收敛曲线** + σ 值
...
面板里**取消勾选"相位"和"偏置"**，选好固定步态即可 —— 维度会从 34 维降到 **7 维**，搜索快得多，结果也更好解释（"在对角步态下，最优频率是 X、幅度是 Y"）。这正是做**步态对比实验**的正确姿势：固定步态族分别优化，再比谁快。
...
| 双击这个 | 作用 |
|---|---|
| `0_安装环境.bat` | 装 Python 环境和 MuJoCo（**第一次只需运行一次**） |
| `1_看示例步态.bat` | 打开 3D 窗口，看机器人用手工步态游 |
| `2_自动寻优.bat` | CMA-ES 自动搜最优泳姿（约 3 分钟，结果存 results/） |
| `3_看最优结果.bat` | 打开窗口，看搜出来的最优泳姿 |
| `4_导入我的模型.bat` | 把你的 URDF 转成本管线可用的模型 |
| `5_边训练边看.bat` | **边寻优边看**：每试一组参数就播放一次，跑完自动换下一组 |
| `6_边训练边看_只看纪录.bat` | 同上，但只回放"刷新纪录"的那几次（省时间，看得清进步） |
...
**① 转换模型**：双击 `4_导入我的模型.bat` → 把你的 `.urdf` 拖进窗口 → 回车
...
**③ 跑**：双击 `1_看示例步态.bat` 先看动起来了没，再 `2_自动寻优.bat`。
